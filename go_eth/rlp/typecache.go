...
import (
	...
	"github.com/RNRetailer/rng/go_eth/rlp/internal/rlpstruct"
)
...
// MUYIWA COMEBACK
// ShallowCloneMap creates a shallow copy of the input map.
func ShallowCloneMap(original map[typekey]*typeinfo) map[typekey]*typeinfo {
	// Create a new map with the same type and capacity
	clone := make(map[typekey]*typeinfo, len(original))

	// Copy all key-value pairs from the original map to the new map
